...
const TESTS: TestData[] = [
  {
    title: "Test 1: Technical & Frontend Core",
    description: "Evaluates your grasp of modern React architecture, React hooks, and database indexing strategies.",
    minScore: 2,
    skillsAffected: ["React", "Database Optimization"],
    questions: [
      {
        id: 1,
        question: "What is the main purpose of React's useMemo hook?",
        options: [
          { key: "A", text: "To make asynchronous HTTP requests." },
          { key: "B", text: "To cache the value of a function execution between renders." },
          { key: "C", text: "To trigger browser layout reflows." },
        ],
        correctOption: "B",
      },
      {
        id: 2,
        question: "Which React hook should be used to perform side effects like subscribing to a data stream?",
        options: [
          { key: "A", text: "useState" },
      ...
      {
        id: 3,
        question: "In a database, how does adding an index affect query performance?",
        options: [
          { key: "A", text: "It slows down reads and speeds up writes." },
          { key: "B", text: "It speeds up reads but can slow down writes." },
          { key: "C", text: "It has no effect on writes." },
        ],
        correctOption: "B",
      },
    ],
  ...
  {
    title: "Test 2: System & Network Architectures",
    description: "Assessments on core C++ memory models, subnet designs, and basic IP networking concepts.",
    minScore: 2,
    skillsAffected: ["C++", "Network Subnetting"],
    questions: [
      {
        id: 1,
        question: "What does the term 'VLAN' stand for in networking?",
        options: [
          { key: "A", text: "Virtual Local Area Network" },
          { key: "B", text: "Vector Line Access Node" },
          { key: "C", text: "Variable Loop Address Number" },
        ],
        correctOption: "A",
      },
      {
        id: 2,
        question: "Which of the following handles automatic, exception-safe memory management in modern C++?",
        options: [
          { key: "A", text: "Raw pointers using new and delete keywords" },
          { key: "B", text: "Smart pointers (like std::unique_ptr and std::shared_ptr)" },
          { key: "C", text: "The automatic garbage collector" },
        ],
        correctOption: "B",
      },
      {
        id: 3,
        question: "In classful networking, which subnet mask corresponds to a Class C address block (/24)?",
        options: [
  ...
  {
    title: "Test 3: Collaborative Conflict Engineering",
    description: "Evaluates soft skills, workplace communications, presentation fundamentals, and dispute resolution.",
    minScore: 2,
    skillsAffected: ["Public Speaking", "Conflict Resolution"],
    questions: [
      {
        id: 1,
        question: "If two team members disagree on a technical design, what is the best first step to resolve conflict?",
        options: [
          { key: "A", text: "Escalate immediately to upper management." },
          { key: "B", text: "Schedule a structured technical discussion to map pros/cons against project requirements." },
          { key: "C", text: "Implement both versions and decide later." },
        ],
        correctOption: "B",
      },
      {
        id: 2,
        question: "What is the most effective way to start a high-stakes presentation to stakeholders?",
        options: [
          { key: "A", text: "Read the agenda slide verbatim." },
          { key: "B", text: "Start with a hook, a clear value proposition, or a key takeaway." },
          { key: "C", text: "Apologize for not having enough time to prepare." },
        ],
        correctOption: "B",
      },
      {
        id: 3,
        question: "Active listening in collaborative environments is best demonstrated by:",
        options: [
          { key: "A", text: "Formulating your rebuttal while the other person is speaking." },
          { key: "B", text: "Summarizing the speaker's main point and validating their perspective before responding." },
          { key: "C", text: "Nodding constantly and changing the subject immediately." },
        ],
        correctOption: "B",
      },
    ],
...
export default function StudentHub() {
  ...
  const [skills, setSkills] = useState<Skill[]>([
    { name: "React", value: 50, type: "hard" },
    { name: "C++", value: 50, type: "hard" },
    { name: "Network Subnetting", value: 50, type: "hard" },
    { name: "Database Optimization", value: 50, type: "hard" },
    { name: "Public Speaking", value: 50, type: "behavioral" },
    { name: "Conflict Resolution", value: 50, type: "behavioral" },
  ]);

  // Overall Readiness Score (Elo style)
  const [readinessScore, setReadinessScore] = useState(1200);
  ...
  useEffect(() => {
    const savedSkillMap = localStorage.getItem("careeros-skill-map");
    const completed = localStorage.getItem("careeros-assessment-completed") === "true";
    const savedElo = localStorage.getItem("careeros-readiness-score");

    if (savedSkillMap) {
      try {
        setSkills(JSON.parse(savedSkillMap));
      } catch (e) {
        console.error("Failed to parse saved skill map", e);
      }
    } else {
      // Create baseline skill map (baseline score of 50% for all)
      const defaultSkillMap = [
        { name: "React", value: 50, type: "hard" as const },
        { name: "C++", value: 50, type: "hard" as const },
        { name: "Network Subnetting", value: 50, type: "hard" as const },
        { name: "Database Optimization", value: 50, type: "hard" as const },
        { name: "Public Speaking", value: 50, type: "behavioral" as const },
        { name: "Conflict Resolution", value: 50, type: "behavioral" as const },
      ];
      localStorage.setItem("careeros-skill-map", JSON.stringify(defaultSkillMap));
      setSkills(defaultSkillMap);
    }

    if (savedElo) {
      setReadinessScore(parseInt(savedElo, 10));
    } else {
      localStorage.setItem("careeros-readiness-score", "1200");
      setReadinessScore(1200);
    }
  ...
  if (!hasLoaded) {
    return (
      <div className="flex items-center justify-center min-h-[600px] bg-slate-950 text-slate-400 font-mono">
        <div className="space-y-4 text-center">
          <div className="h-8 w-8 border-2 border-teal-500 border-t-transparent animate-spin rounded-full mx-auto" />
          <div className="text-xs tracking-widest animate-pulse">SYNCHRONIZING SECURE LEDGER...</div>
        </div>
      </div>
    );
  }
  ...
  if (!assessmentCompleted) {
    return (
      <div className="space-y-8 bg-slate-950 p-4 sm:p-6 lg:p-8 rounded-2xl text-slate-100 border border-slate-900 shadow-2xl relative overflow-hidden font-mono min-h-[600px]">
        {/* Futuristic Style Sheet (same as main) */}
        ...
        <div className="relative border-b border-slate-800/80 pb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 glass-gradient-teal">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-teal-400 font-mono text-xs tracking-wider uppercase">
              ...
              Neural Skill Alignment Onboarding // Diagnostics Mode
            </div>
            <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-teal-400 via-cyan-300 to-purple-400 bg-clip-text text-transparent">
              Capabilities Onboarding Assessment
            </h1>
            <p className="text-slate-400 text-sm max-w-2xl font-sans">
              Establish your verified baseline index across 6 core skill axes before connecting to the CareerOS matchmaking protocol.
            </p>
          </div>
        </div>

        {/* Onboarding HUD Content */}
        {showIntro ? (
          /* Welcome Intro View */
          <div className="cyber-card rounded-xl p-8 max-w-2xl mx-auto space-y-6 text-center border-slate-800 relative">
            <div className="absolute top-0 right-0 h-[1px] w-1/3 bg-gradient-to-l from-teal-500 to-transparent" />
            <Sparkles className="h-12 w-12 text-teal-400 mx-auto animate-pulse" />
            
            <div className="space-y-2">
              <h2 className="text-xl font-bold text-slate-100">Welcome, Candidate</h2>
              <p className="text-xs text-slate-400 leading-relaxed font-sans">
                To unlock your matching capabilities dashboard and interact with work-integrated learning projects, you must complete three diagnostic tests. Your performances directly establish your skill map ratings.
              </p>
            </div>

            <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-5 text-left text-xs space-y-4">
              <div className="font-semibold text-teal-300 flex items-center gap-2">
                <Info className="h-4 w-4 shrink-0" />
                Assessment Instructions
              </div>
              <ul className="list-disc list-inside space-y-2 text-slate-300 font-mono leading-relaxed">
                <li>Three sequential tests focusing on Frontend, Systems/Networking, and Behavioral capabilities.</li>
                <li>Each test has 3 questions and requires a score of <span className="text-teal-400 font-semibold">at least 2/3</span> to pass.</li>
                <li>Passing a test updates your profile's skill map and unlocks access to the next test.</li>
                <li>If you fail, you can retry as many times as necessary to proceed.</li>
              </ul>
            </div>

            <Button
              onClick={() => setShowIntro(false)}
              className="w-full bg-teal-600 hover:bg-teal-500 text-slate-950 font-bold h-11 shadow-[0_0_15px_rgba(13,148,136,0.3)] transition-all uppercase tracking-wider"
            >
              Initialize Assessment Sequence
            </Button>
          </div>
        ) : (
          /* Active Test Onboarding View */
          <div className="grid gap-6 md:grid-cols-12">
            {/* Left Column: Progress Sidebar (4 cols) */}
            <div className="md:col-span-4 space-y-4">
              <div className="cyber-card rounded-xl p-5 border-slate-800 h-full flex flex-col justify-between">
                <div className="space-y-4">
                  <h3 className="text-xs font-semibold text-slate-400 font-mono tracking-wider uppercase border-b border-slate-800/60 pb-2">
                    Sequence Status
                  </h3>
                  
                  <div className="space-y-6 relative mt-4 pl-4">
                    {/* Progress vertical timeline line */}
                    <div className="absolute left-[23px] top-2 bottom-2 w-[2px] bg-slate-800" />

                    {TESTS.map((test, index) => {
                      const isActive = index === currentTestIndex;
                      const isCompleted = index < currentTestIndex;
                      const isLocked = index > currentTestIndex;

                      return (
                        <div key={index} className="flex gap-4 items-start relative">
                          <div className="relative z-10">
                            {isCompleted ? (
                              <div className="h-5 w-5 rounded-full bg-emerald-950 border border-emerald-500 flex items-center justify-center shadow-[0_0_8px_rgba(16,185,129,0.3)]">
                                <CheckCircle2 className="h-3 w-3 text-emerald-400" />
                              </div>
                            ) : isActive ? (
                              <div className="h-5 w-5 rounded-full bg-teal-950 border border-teal-400 flex items-center justify-center shadow-[0_0_8px_rgba(13,148,136,0.5)]">
                                <div className="h-1.5 w-1.5 rounded-full bg-teal-400" />
                              </div>
                            ) : (
                              <div className="h-5 w-5 rounded-full bg-slate-950 border border-slate-800 flex items-center justify-center">
                          ...
                          <div className="flex-1 text-xs">
                            <div className={`font-bold font-mono ${isActive ? "text-teal-300" : isCompleted ? "text-emerald-400" : "text-slate-600"}`}>
                              {test.title}
                            </div>
                            <div className="text-[10px] text-slate-500 mt-0.5">
                              {isCompleted ? "Verified Profile Link" : isActive ? "Active Diagnosis" : "Awaiting Unlock"}
                            </div>
                          </div>
                        </div>
                      );
                ...
                <div className="border-t border-slate-800/80 pt-4 mt-6 text-[10px] text-slate-500 leading-normal font-sans">
                  <span className="font-semibold text-slate-400 block mb-1 font-mono">Adaptive Alignment Module</span>
                  Passing grades automatically adjust your base skill stats, providing immediate alignment boosts with incoming employer requests.
                </div>
              </div>
            </div>

            {/* Right Column: Question Panel (8 cols) */}
            <div className="md:col-span-8">
              <div className="cyber-card rounded-xl p-6 border-slate-800 min-h-[400px] flex flex-col justify-between relative">
                <div className="absolute top-0 right-0 h-[1px] w-1/3 bg-gradient-to-l from-teal-500 to-transparent" />
                
                {/* Result View or Question Form */}
                {testResult ? (
                  /* Test Result Screen */
                  <div className="space-y-6 my-auto text-center py-6">
                    {testResult.passed ? (
                      <div className="space-y-4">
                        ...
                        <h2 className="text-xl font-bold text-emerald-400 uppercase tracking-wider">Test Completed Successfully!</h2>
                        <p className="text-sm text-slate-300 max-w-md mx-auto font-sans">
                          You scored <span className="font-bold text-emerald-400 font-mono">{testResult.score}/3</span>. This meets the baseline credentials requirements!
                        </p>
                        <div className="bg-slate-950/60 border border-slate-900 rounded-lg p-4 max-w-sm mx-auto text-left text-xs font-mono space-y-1 text-slate-400">
                          <span className="text-teal-400 font-semibold block mb-1">Updated Skill Map Indexes:</span>
                          {TESTS[currentTestIndex].skillsAffected.map(skillName => {
                            const val = skills.find(s => s.name === skillName)?.value || 0;
                            return (
                        ...
                        <Button
                          onClick={() => {
                            if (currentTestIndex + 1 < TESTS.length) {
                              ...
                              localStorage.setItem("careeros-assessment-completed", "true");
                              setAssessmentCompleted(true);
                              toast.success("All diagnostic tests passed! Profile fully unlocked.");
                            }
                          }}
                          className="bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-bold px-8 h-10 shadow-[0_0_10px_rgba(16,185,129,0.3)] mt-4"
                        >
                          {currentTestIndex + 1 < TESTS.length ? "Proceed to Next Test" : "Unlock CareerOS Dashboard"}
                        </Button>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <X className="h-14 w-14 text-red-500 mx-auto animate-pulse" />
                        <h2 className="text-xl font-bold text-red-400 uppercase tracking-wider">Test Minimum Not Reached</h2>
                        <p className="text-sm text-slate-300 max-w-md mx-auto font-sans">
                          You scored <span className="font-bold text-red-400 font-mono">{testResult.score}/3</span>. You need a minimum of <span className="font-semibold text-slate-200">2/3 correct answers</span> to pass this phase and proceed.
                        </p>
                        <p className="text-xs text-slate-500 font-sans">
                          Review standard definitions and documentation. Retakes are free and unlimited.
                        </p>
                        <Button
                          onClick={() => {
                            setSelectedAnswers({});
                          ...
                          className="bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold px-8 h-10 border border-slate-700 mt-4"
                        >
                          Retry Test
                        </Button>
                      </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      <span className="text-[10px] text-teal-400 font-mono tracking-widest uppercase block mb-1">
                        Phase {currentTestIndex + 1} of 3
                      </span>
                      <h2 className="text-lg font-bold text-slate-100">{TESTS[currentTestIndex].title}</h2>
                      <p className="text-xs text-slate-400 mt-1 font-sans">{TESTS[currentTestIndex].description}</p>
                    </div>

                    <div className="space-y-6 border-t border-slate-800/60 pt-4">
                      {TESTS[currentTestIndex].questions.map((q, qIdx) => (
                        <div key={q.id} className="space-y-2.5">
                          <div className="text-xs font-semibold text-slate-200 flex gap-2 font-sans">
                            <span className="text-teal-400 font-mono">{qIdx + 1}.</span>
                            ...
                            {q.options.map((opt) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`w-full text-left px-3 py-2 text-xs rounded border font-mono transition-all flex items-center gap-3 ${
                                    isSelected 
                                      ? "bg-teal-950/40 border-teal-500 text-teal-300 shadow-[0_0_10px_rgba(13,148,136,0.15)]" 
                                ...
                                >
                                  <span className={`h-4 w-4 rounded-full border flex items-center justify-center text-[10px] shrink-0 ${
                                    isSelected ? "border-teal-400 text-teal-300 font-bold" : "border-slate-700 text-slate-600"
                                  }`}>
                                    {opt.key}
                                  </span>
                                  <span>{opt.text}</span>
                    ...
                    <div className="border-t border-slate-800/80 pt-4 mt-6">
                      <Button
                        onClick={() => {
                          ...
                          const passed = correctCount >= test.minScore;
                          
                          if (passed) {
                            // Update skills
                            const multiplier = correctCount === 3 ? 1.0 : 0.8;
                            const newSkills = skills.map(skill => {
                              if (test.skillsAffected.includes(skill.name)) {
                                const baseVal = skill.name === "React" || skill.name === "Network Subnetting" || skill.name === "Conflict Resolution" ? 95 : 90;
                                return {
                                  ...skill,
                                  value: Math.round(baseVal * multiplier)
                                };
                              }
                              return skill;
                            });
                            ...
                            // Update Elo
                            const eloBonus = correctCount === 3 ? 150 : 100;
                            const newElo = readinessScore + eloBonus;

                            setSkills(newSkills);
                            setReadinessScore(newElo);

                            localStorage.setItem("careeros-skill-map", JSON.stringify(newSkills));
                            localStorage.setItem("careeros-readiness-score", String(newElo));
                          }

                          setTestResult({ score: correctCount, passed });
                        }}
                        className="w-full bg-teal-600 hover:bg-teal-500 text-slate-950 font-bold h-10 tracking-wide uppercase text-xs"
                      >
                        Submit Diagnosis
                      </Button>
                    </div>
    ...
    <div className="space-y-8 bg-slate-950 p-4 sm:p-6 lg:p-8 rounded-2xl text-slate-100 border border-slate-900 shadow-2xl relative overflow-hidden">
      ...
      <div className="relative border-b border-slate-800/80 pb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 glass-gradient-teal">
        ...
        <div className="flex items-center gap-3">
          <Button
            onClick={() => {
              if (confirm("Are you sure you want to reset your skills mapping and retake the diagnostic tests?")) {
                localStorage.removeItem("careeros-skill-map");
                localStorage.removeItem("careeros-readiness-score");
                localStorage.removeItem("careeros-assessment-completed");
                ...
                setSelectedAnswers({});
                setTestResult(null);
                toast.success("Assessment mapping reset. Returning to Diagnostic Phase.");
              }
            }}
            variant="outline"
            className="border-slate-800 hover:bg-slate-900 font-mono text-xs text-slate-400 hover:text-slate-100 flex items-center gap-1.5 h-8"
          >
            <Activity className="h-3.5 w-3.5" /> Retake Tests
          </Button>
